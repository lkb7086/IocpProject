...
std::string CMinidumpHelp::format_arg_list(const char *fmt, va_list args)
{
	if (!fmt) return "";
...
std::string CMinidumpHelp::format_string(const char *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
...
std::wstring CMinidumpHelp::s2ws(const std::string& s)
{
	int len;
...
std::wstring CMinidumpHelp::get_dump_filename()
{
	time_t rawtime;
	struct tm timeinfo;
...
LONG WINAPI my_top_level_filter(__in PEXCEPTION_POINTERS pExceptionPointer)
{
	...
	// https://docs.microsoft.com/ko-kr/windows/win32/api/minidumpapiset/ne-minidumpapiset-minidump_type
...
void CMinidumpHelp::install_self_mini_dump()
{
